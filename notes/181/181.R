...
# using data from the results section (reading achivements):
mean_int_pre <- 64.01
sd_int_pre <- 32.83
mean_int_post <- 70.57
sd_int_post <- 30.41
...
# calculate SMD
# using total N data (must use exact = FALSE because of overflow)
smd(mean_int_d, mean_con_d, sd_int_d, sd_con_d, 156, 304, exact = FALSE) |> get_ci()
# very near (off by 0.01)

# using n of completed tests
smd(mean_int_d, mean_con_d, sd_int_d, sd_con_d, 153, 203, exact = FALSE) |> get_ci()
# bit more off
...
smd(db_mean_int_post, db_mean_con_post, db_sd_int_post, db_sd_con_post, 417, 214, exact = FALSE) |> get_ci()
...
smd(db_mean_int_d, db_mean_con_d, db_sd_int_d, db_sd_con_d, 417, 214, exact = FALSE) |> get_ci()
# perfect match!
