...
*TO-DO*
<!-- Jenny Murphy^1^, Simon Nolte^2,3^, Oliver Jan Quittmann^3^, Robert Rein^2^, Theresa Siepe^3^, ??? -->
<!-- ^1^School of Biological, Health and Sport Sciences, Technological University Dublin, Tallaght, Dublin, Ireland ^2^Institute of Exercise Training and Sport Informatics, German Sport University Cologne ^2^Institute of Movement and Neurosciences, German Sport University Cologne -->

### Tasks and Roles

*TO-DO*

Conceptualization; Data curation; Formal Analysis; Software; Investigation; Methodology; Visualization; Writing;

## Review methods

### Type of Review

Methodological review, with additional analysis of an example case (see @sec-example).
...
Effect size determination method used.
...
-   Criteria/assessment of baseline imbalances for studies included in the meta-analysis
-   Effect size measure (mean difference, standardized mean difference)
...
-   Only research published in 2024 (date of print publication). Note that this may be expanded based on the number of included articles (see @sec-samplesize).
...
Replace `_JOURNALNAMEX_` by the name of each journal. Adjust date range if necessary (see section @sec-samplesize)
...
We will not contact any author during the review. We may contact authors during the case study (see @sec-example).
...
2.  Full-text retrieval and screening
...
We will store the initial search results in a `csv` file. Results of screening and data extraction will be stored in the data base back-end of the web platform and can be exported. All data and analysis code will be made publicly available on GitHub.
...
![Extraction criteria and instructions in the review web platform](extraction_instructions.png){#fig-extract fig-align="center"}

### Extraction stages

Data extraction is conducted in three stages:

1.  **Full-text data extraction:** One researcher extracts the reported characteristics of each article.
2.  **Recalculation:** The same researcher attempts to recalculate an effect size for one study included in the meta-analysis. This study is selected as the first (top) study in the first forest plot that meets the screening criteria (i.e., a forest plot representing a pre-post group comparison design). If a different study is used for the reproducibility check, the reason is documented. If individual study effect sizes are not presented in the meta-analysis, recalculation is deemed infeasible.
3.  **Validation:** An independent researcher reviews the extraction results. Using an annotated article full text, this researcher decides whether to validate the initial data extraction or to suggest changes. If changes are suggested, the article returns to Stage 1, where the original researcher re-evaluates their extraction results.
...
The researcher performing the validation is blinded to who performed the data extraction. Otherwise, no blinding applies.

### Extraction reliability

We perform a validation stage to check for potential issues in the data extraction process.
...
For economic reasons, a single researcher will perform the initial data extraction. To minimize errors, we use an independent validation procedure. Additionally, we implement a recalculation stage to ensure that the reported analytical characteristics align with those actually applied in the included studies.
...
For the case study, we use the article by @oliveira2024, as it served as the motivating example for the project lead (SN) to initiate this research project. While reviewing the meta-analysis, he identified a triple-counting of studies in the forest plot of the article's main finding (Fig. 4 in @oliveira2024). Additionally, attempts to reproduce the meta-analysis revealed discrepancies, as the standard deviations of change scores reported in the meta-analysis could not be directly reproduced. Through communication with the lead author, he was able to reconstruct the (previously unreported) method used by the authors. However, this raised further questions about whether altering the methods for effect size calculation might influence the conclusions of the meta-analysis.

### Data Extraction

We will extract all reported summary measures from the original studies included in Fig. 4 of @oliveira2024. For missing summary measures (e.g., pre-post correlations or change score standard deviations) and individual data points, we will attempt to obtain the necessary information by extracting data from figures and/or contacting the authors of the respective articles.
...
For each effect size calculation method, we will calculate the overall mean effect size and its 95%-confidence interval. These results will be compared to evaluate the original research hypothesis by determining whether the confidence intervals overlap with an effect size of zero.
